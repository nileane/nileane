$(function() {
	...
	*/
	...
	success: function( json ) {
		...
		if (json.length == 0) {
			$('#comments').html('<div class="comment" style="display:none;">~</div>');
		}
	},
	...
	complete: function( xhr, status ) {
		$('#comments').append('<a id="comment_addcomment" class="comment_addcomment" style="display:none" href="javascript:toggle(\'comment_form\');toggle(\'comment_addcomment\')">Réagir à ce billet</a><form method="post" action="' + gurl + '?a=p" id="comment_form" class="comment_form" style="display:none"><input type="hidden" value="' + document.URL + '" name="url"><div class="comment_inputuser"><input type="text" value="" placeholder="votre pseudonyme" size="20" name="name"></div><div class="comment_inputmessage"><textarea placeholder="votre commentaire…" value="" name="comment" cols="32" rows="2"></textarea></div><div class="comment_recaptcha"><input type="text" placeholder="Copiez le code" name="captcha"><a title="Recharger l\'image" href="javascript:reloadCaptcha()"><img id="captcha" alt="Copiez le code" src="' + gurl + '?a=c"></a><br><small style="color:#666;">les lettres du code sont en majuscule.</small></div><div class="comment_submit"><input type="submit" value="Envoyer" name="submit"></div></form>')
